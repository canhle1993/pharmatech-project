...
export class AboutComponent implements OnInit {
    ...
    this.aboutService.getAbout().subscribe((data: any) => {
      if (data) {
        ...
        if (data.testimonials && Array.isArray(data.testimonials)) {
          ...
          data.testimonials.forEach((testimonial: any) => this.addTestimonial(testimonial));
        }
    ...
    if (input.files) {
      ...
      for (const file of Array.from(input.files)) {
        ...
        if (type === 'brands') {
          ...
          if (!isValid) {
            alert(`Brand logo "${file.name}" is too large. Maximum size is 300x300 pixels`);
            continue;
          }
        }
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (this.selectedImages['introSignature']) {
        const url = await this.uploadImage(this.selectedImages['introSignature'], 'intro');
        introData.signatureImage = url;
      }
      if (this.selectedImages['introImage']) {
        const url = await this.uploadImage(this.selectedImages['introImage'], 'intro');
        introData.image = url;
      }
      ...
      if (this.selectedImages['openImage']) {
        const url = await this.uploadImage(this.selectedImages['openImage'], 'open');
        openData.image = url;
      }
      ...
      if (formValue.testimonialsSection) {
        formData.append('testimonialsSection', JSON.stringify(formValue.testimonialsSection));
      }
        ...
        const teamData = await Promise.all(
          formValue.team.map(async (member: any, index: number) => {
            if (this.selectedImages['team' + index]) {
              const imageUrl = await this.uploadImage(this.selectedImages['team' + index], 'team');
      ...
      if (formValue.testimonials) {
        const testimonialsData = await Promise.all(
          formValue.testimonials.map(async (testimonial: any, index: number) => {
            if (this.selectedImages['testimonial' + index]) {
              const imageUrl = await this.uploadImage(this.selectedImages['testimonial' + index], 'testimonials');
      ...
      if (formValue.brandImages && formValue.brandImages.length > 0) {
        const uploadedBrandImages = await Promise.all(
          formValue.brandImages.map(async (imageData: string, index: number) => {
            // If it's a data URL (newly uploaded), upload it to server
            if (imageData.startsWith('data:')) {
              // Convert data URL to File
              const blob = await fetch(imageData).then(r => r.blob());
              const file = new File([blob], `brand-${Date.now()}-${index}.png`, { type: blob.type });
      ...
      if (this.currentId) {
        await firstValueFrom(this.aboutService.updateAbout(this.currentId, formData));
      } else {
      ...
      const message = error?.error?.message || error?.message || 'There was an error saving the data. Please try again.';
      alert('Error: ' + message);
    } finally {
  ...
  private async uploadImage(file: File, folder: string): Promise<string> {
    ...
    try {
      ...
      const response = await firstValueFrom(this.aboutService.uploadImage(formData));
      return response.url;
    } catch (error) {
  ...
  private checkImageDimensions(file: File, expectedWidth: number, expectedHeight: number): Promise<boolean> {
  ...
  private checkImageMaxDimensions(file: File, maxWidth: number, maxHeight: number): Promise<boolean> {
    return new Promise((resolve) => {
      const img = new Image();
      img.onload = () => {
        URL.revokeObjectURL(img.src);
