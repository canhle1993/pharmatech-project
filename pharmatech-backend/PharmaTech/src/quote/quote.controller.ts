...
export class QuoteController {
  ...
  async sendReply(@Param('id') id: string, @Body() replyDto: ReplyQuoteDto) {
    try {
      console.log('Reply endpoint called with id:', id);
      console.log('Request body:', JSON.stringify(replyDto));
      console.log('replyMessage:', replyDto.replyMessage);
      console.log('message length:', replyDto.replyMessage?.length);
      
      // Strip HTML tags to validate actual text content
      const stripHtml = (html: string) => html.replace(/<[^>]*>/g, '').trim();
      const textContent = replyDto.replyMessage ? stripHtml(replyDto.replyMessage) : '';
      ...
      const result = await this.quoteService.sendReply(id, replyDto.replyMessage);
      ...
      console.error('Error in reply controller:', error);
