    ...
    try {
      ...
      if (this.selectedImages['banner']) {
        const uploadResult = await firstValueFrom(
          this.contactService.uploadImage(this.selectedImages['banner'], 'contact-banner')
        );
        formData.append('bannerImage', uploadResult.url);
      } else if (this.contactForm.get('bannerImage')?.value) {
        formData.append('bannerImage', this.contactForm.get('bannerImage')?.value);
      }
      ...
      // Submit
      if (this.currentId) {
        await firstValueFrom(this.contactService.updateContact(this.currentId, formData));
        alert('Contact information updated successfully!');
      } else {
        const result = await firstValueFrom(this.contactService.createContact(formData));
