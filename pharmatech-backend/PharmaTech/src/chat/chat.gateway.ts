...
export class ChatGateWay {
  ...
  joinAdmin(@MessageBody() payload: { adminId: string }, @ConnectedSocket() client: Socket) {
  ...
  async loadInbox(@MessageBody() payload: { limit?: number; cursor?: string }) {
    const rows = await this.chatService.listInbox(payload?.limit ?? 50, payload?.cursor);
    return rows;
  }
 ...
 // nếu bạn phân biệt được thì thay logic này
    ...
    if (isFromUser) {
      const updatedInbox = await this.chatService.upsertInboxOnIncomingFromUser(
        payload.userId, text, fromId, createdAt,
      );
      this.server.emit('inbox:upsert', updatedInbox); // đẩy lên sidebar admin
    } else {
      const updatedInbox = await this.chatService.upsertInboxOnOutgoingFromAdmin(
        payload.userId, text, fromId, createdAt,
      );
